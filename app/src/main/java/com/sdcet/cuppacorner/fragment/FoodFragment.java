...
import com.sdcet.cuppacorner.adapter.entity.Food2;
import com.sdcet.cuppacorner.core.BaseFragment;
import com.sdcet.cuppacorner.adapter.base.ShequRecyclerAdapter;
import com.sdcet.cuppacorner.adapter.entity.Food;
import com.sdcet.cuppacorner.utils.TokenUtils;
import com.xuexiang.cuppacorner.databinding.FragmentFoodBinding;
...
@Page
public class FoodFragment extends BaseFragment<FragmentFoodBinding> {
   ...
   @NonNull
   @Override
   protected FragmentFoodBinding viewBindingInflate(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, boolean attachToRoot) {
      return FragmentFoodBinding.inflate(inflater, container, attachToRoot);
   }

   @Override
   protected void initViews() {
      recyclerView = binding.case56Rv;
      if (recyclerView == null) {
         Log.e("FoodFragment", "RecyclerView is null");
         return;
      }
      ...
      XHttp.get("androidapi/loadFood")
              .headers("Authorization", "Bearer " + token)
              .syncRequest(false) //异步请求
              .onMainThread(true) //回到主线程
              .execute(new SimpleCallBack<List<Food2>>() {
                 @Override
                 public void onSuccess(List<Food2> response) throws Throwable {
                    if (response != null && response.size() > 0) {
                       for (Food2 item : response) {
                          Food food = new Food();
                           food.setName(item.getName());
                           food.setImageUrl(item.getImageUrl());
                           food.setDetailUrl(item.getDetailUrl());
                          foodList.add(food);
                          Log.e("foodlist",foodList.toString());
                       }
                       adapter = new ShequRecyclerAdapter(foodList); //重新设置适配器
